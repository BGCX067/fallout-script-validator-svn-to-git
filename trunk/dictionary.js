...
function scriptFunctionParam(dataType, name, optional, values, deprecated) {
	...
	this.name = name;
	this.optional = false || !!optional; //False unless included, otherwise true

	this.values = false || values; //False unless included, otherwise as specified
	this.deprecated = false || !!deprecated; //False unless included, otherwise true
}
...
//Keywords
{
	...
	dictionary.keywords['int'].contextual_validate = function(script) {
		if (script.inBlock != '') {
			script.pushWarning('Best practice mandates that variable declarations should take outside Begin/End blocks, ideally at the top of the script.');
		}
	...
	dictionary.keywords['float'].contextual_validate = function(script) {
		if (script.inBlock != '') {
			script.pushError('Best practice mandates that variable declarations should take outside Begin/End blocks, ideally at the top of the script.');
		}
...
//Blocktypes
{
	...
	//New Vegas
	{
		$.extend(dictionary.newvegasblocktypes, dictionary.fallout3blocktypes);
	}

	$.extend(dictionary.blocktypes, dictionary.fallout3blocktypes);
}

//Fallout 3 Values
{
	dictionary.fallout3values.attributes = {
		strength: 'Strength',
	...
	dictionary.fallout3values.skills = {
		barter: 'Barter',
		bigguns: 'BigGuns',
	...
	dictionary.fallout3values.actorValues = {
	...
	dictionary.fallout3values.axes = { x: 'X', y: 'Y', z: 'Z' };

	dictionary.fallout3values.menuModes = {
		0: '0',
		1: '1',
		2: '2',
		3: '3',
...
//New Vegas Values
{
	$.extend(dictionary.newvegasvalues, dictionary.fallout3values);
	$.extend(dictionary.newvegasvalues.actorValues, {survival: 'Survival'});
}

$.extend(dictionary.values, dictionary.fallout3values);
...
//Fallout 3 Functions
{
	...
	dictionary.fallout3functions['setunconscious'] = new scriptFunction('void', 'R', 0, [new scriptFunctionParam('int', 'UnconsciousFlag')], {docLink: 'http://geck.bethsoft.com/index.php/SetUnconscious', name: 'SetUnconscious'});
...
//FOSE Functions
{
	...
	dictionary.fosefunctions['goto'] = new scriptFunction('void', 'B', 1.00, [new scriptFunctionParam('int', 'labelID')], {docLink: 'http://fose.silverlock.org/fose_command_doc.html#Goto', name: 'Goto'}, [new scriptFunctionNote(
		function (functionCall) {
			if (functionCall[1] != null) {
				if (parseInt(functionCall[1], 10).toString() !== functionCall[1]) {
					return true;
				}
			}
			return false;
		},
		'Best practise mandates that an hard-coded integer value is used for the labelID parameter of label and goto',
		0
	), new scriptFunctionNote(
		function (functionCall) {
			var label = parseInt(functionCall[1], 10);
			if (label.toString() === functionCall[1]) {
				return label < 0 || label > 255;
			} else {
				return false;
			}
		},
		'The labelID parameter must be between 0 and 255, inclusive.',
		1
	)]);
	...
	dictionary.fosefunctions['label'] = new scriptFunction('void', 'B', 1.00, [new scriptFunctionParam('int', 'labelID')], {docLink: 'http://fose.silverlock.org/fose_command_doc.html#Label', name: 'Label'}, [new scriptFunctionNote(
		function (functionCall) {
			if (functionCall[1] != null) {
				if (parseInt(functionCall[1], 10).toString() !== functionCall[1]) {
					return true;
...
//New Vegas Functions
{
	$.extend(dictionary.newvegasfunctions, dictionary.fallout3functions);
}

//NVSE Functions
{
	$.extend(dictionary.nvsefunctions, dictionary.fosefunctions);
}

$.extend(dictionary.functions, dictionary.fallout3functions, dictionary.fosefunctions);