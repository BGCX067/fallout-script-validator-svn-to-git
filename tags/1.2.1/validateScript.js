...
function highlight(string)
{
	...
	string = string.match(/".*?("|$)|'.*?('|$)|[^"']+/g); //Separate string literals from the rest of the input string
		...
		if (string[i][0] == '"' || string[i][0] == "'") //String
			...
			for (var j = 0, jMax = string[i].length; j < jMax; j++) { //Loop through words in substrings
				...
				//Functions
					...
					var functionDoc = functionDef.returnType + ' ' + (functionDef.callingConvention in {'R': 1, 'E': 1} ? '[ref].' : '') + '<span class="' + (functionDef.SEVersion ? 'ScriptExtender' : 'GECK') + 'Function">' + functionDef.name + '</span>';
					if (typeof functionDef.params != 'undefined') {
					...
					string[i][j] = (typeof functionDef.docLink == 'undefined' ? '<span' : '<a href="' + functionDef.docLink + '"') + ' class="' + (functionDef.SEVersion ? 'ScriptExtender' : 'GECK');
					string[i][j] += 'Function"><span class="functionInfoWrapper"><span class="functionInfo">' + functionDoc + '</span>';
					string[i][j] += functionName + '</span>' + (typeof functionDef.docLink == 'undefined' ? '</span>' : '</a>');
...
try {
	script = {
		...
		errors: [],
		warnings: [],
		input: trim(inputElement.value ? inputElement.value : inputElement.innerHTML).split(/\n|<br\/?>/i),
		output: [],
	...
	$('.functionInfoWrapper').hover(function() {if (document.getElementById('tooltips').checked) {$(this).children('.functionInfo').toggle();}});
} catch(e) {
